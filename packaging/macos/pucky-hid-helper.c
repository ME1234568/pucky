...
/*
 * Xbox One S (Model 1708) Bluetooth HID descriptor. Unlike a generic gamepad
 * masquerading under an Xbox VID/PID, this matches the packet format expected
 * by macOS's Xbox controller path: report 1 contains four unsigned stick axes,
 * two 10-bit triggers, the hat, buttons, and Back; report 2 carries Guide.
 */
static const uint8_t report_descriptor[] = {
    ...
    
};
...
static CFTypeRef create_gamepad(void)
{
    ...
    set_number(properties, CFSTR("ProductID"), 0x02FD);
    set_number(properties, CFSTR("VersionNumber"), 0x0903);
...
int main(void)
{
    ...
    for (;;) {
        ...
        if (used == REPORT_LENGTH) {
            int32_t result = handle_report(
                device,
                report,
                XBOX_STATE_REPORT_LENGTH);
            if (result != 0) {
                fprintf(
                    stderr,
                    "IOHIDUserDeviceHandleReport (state) failed: 0x%08x\n",
                    (unsigned int)result);
            }

            uint8_t guide = report[XBOX_STATE_REPORT_LENGTH] != 0;
            if (guide != last_guide) {
                const uint8_t guide_report[] = { 0x02, guide };
                result = handle_report(
                    device,
                    guide_report,
                    (CFIndex)sizeof(guide_report));
                if (result != 0) {
                    fprintf(
                        stderr,
                        "IOHIDUserDeviceHandleReport (Guide) failed: 0x%08x\n",
                        (unsigned int)result);
                }
                last_guide = guide;
            }
