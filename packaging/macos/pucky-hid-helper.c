...
/*
 * Razer Serval-compatible raw HID descriptor using the sparse button and axis
 * layout expected by Chromium's macOS standard-gamepad mapper. An Xbox VID/PID
 * cannot be used here: macOS routes recognized Xbox identities through its
 * GameController driver, which does not consume arbitrary IOHIDUserDevice
 * trigger reports. There are no report IDs, so a packet is exactly 15 bytes.
 */
